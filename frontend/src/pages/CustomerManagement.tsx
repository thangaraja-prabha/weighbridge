...
const CustomerManagement: React.FC = () => {
    ...
    const [formData, setFormData] = useState({ cname: '', cadd: '', cnum: '', crem: '' });
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const res = await masterApi.getCustomers(search, 1, 100);
    ...
    const handleAdd = () => {
        setEditing(null);
        setFormData({ cname: '', cadd: '', cnum: '', crem: '' });
        setShowDialog(true);
    };

    const handleEdit = (item: Customer) => {
        setEditing(item);
        setFormData({ cname: item.cname, cadd: item.cadd || '', cnum: item.cnum || '', crem: item.crem || '' });
        setShowDialog(true);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editing) {
                await masterApi.updateCustomer(editing.id, formData);
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            ...
                            {!loading && customers.length === 0 && <tr><td colSpan={4} className="text-center py-4 text-gray-500">No customers found</td></tr>}
                            {!loading && customers.map((item) => (
                                <tr key={item.id}>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.cname}</td>
                                    <td className="px-6 py-4 text-sm text-gray-900 max-w-xs truncate">{item.cadd || '-'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.cnum || '-'}</td>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                ...
                                <input type="text" required className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"
                                    value={formData.cname} onChange={(e) => setFormData({ ...formData, cname: e.target.value })} />
                            </div>
                            <div>
                                ...
                                <textarea className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"
                                    value={formData.cadd} onChange={(e) => setFormData({ ...formData, cadd: e.target.value })} rows={2} />
                            </div>
                            <div>
                                ...
                                <input type="text" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"
                                    value={formData.cnum} onChange={(e) => setFormData({ ...formData, cnum: e.target.value })} />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Remarks</label>
                                <textarea className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"
                                    value={formData.crem} onChange={(e) => setFormData({ ...formData, crem: e.target.value })} rows={2} />
                            </div>
