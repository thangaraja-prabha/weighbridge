...
<tbody>
<tr>
<td align="center" bgcolor="A7AFB0" colspan=4><strong>FIRST WEIGHTMENT</strong></td>
</tr>
</thead>
<tbody>
<tr>
...
//$wt1 = $_REQUEST['wt1'];
 $wt1 = '56675';
 $wt1at = date("Y-m-d H:i:s");
//$stime = date('H:i:s', time());
//echo"$stime";
//exit;
 $wt1by =$_SESSION['username'];
//$bdtime = $_REQUEST['bdtime'];	
//$sused = $_REQUEST['sused'];
 $remarks = $_REQUEST['remarks'];
 $stat = 'W1';
//$stat = 'Pending';
 ...
 $username = $_SESSION['username'];
//exit;
$ins_query="insert into wlog (`vnum`,`mname`,`tname`,`sname`,`cname`,`wt1`,`wt1at`,`wt1by`,`remarks`,`stat`,`username`,`trn_date`) 
values ('$vnum','$mname','$tname','$sname','$cname','$wt1','$wt1at','$wt1by','$remarks','$stat','$username','$trn_date')";
		mysqli_query($con,$ins_query);
		$status = "Updated Sucessfully.</br></br>";	
	//	require_once 'bdu.php';		
}
...
<tr>
<td>Ticket No</td>
<td><?php require('../db.php'); $sql=mysqli_query($con,"select * from wlog Order by id desc limit 1");
	while($res=mysqli_fetch_assoc($sql))
	{
	$id=$res['id']; } echo ($id + 1); ?></td></tr>
<tr>
<td>Vehicle Number</td>
<td><select name="vnum" required >
    <option value required>-- Select --</option>
    <?php
        include "../db.php";  // Using database connection file here
$id=$sessionId;
$query = "SELECT * from vdetail where id='".$id."'"; 
$result = mysqli_query($con, $query) or die ( mysqli_error());
$row = mysqli_fetch_assoc($result);
{
	$apikey = $row['apikey'];

	$records = mysqli_query($con, "SELECT vnum From vdetail group by vnum ");  // Use select query here 

        while($data = mysqli_fetch_array($records))
        {
            echo "<option value='". $data['vnum'] ."'>" .$data['vnum'] ."</option>";  // displaying data in option menu
        }	
}
    ?>  
</select>
...
$row = mysqli_fetch_assoc($result);
{
	...
	$records = mysqli_query($con, "SELECT mname From mdetail group by mname ");  // Use select query here 

        while($data = mysqli_fetch_array($records))
        {
            echo "<option value='". $data['mname'] ."'>" .$data['mname'] ."</option>";  // displaying data in option menu
        }	
}
    ?>  
</select>
...
<select name="tname" required >
    ...
    <?php
        include "../db.php";  // Using database connection file here
$id=$sessionId;
$query = "SELECT * from tdetail where id='".$id."'"; 
$result = mysqli_query($con, $query) or die ( mysqli_error());
$row = mysqli_fetch_assoc($result);
{
	$apikey = $row['apikey'];

	$records = mysqli_query($con, "SELECT tname From tdetail group by tname ");  // Use select query here 
...
$row = mysqli_fetch_assoc($result);
{
	...
	$records = mysqli_query($con, "SELECT sname From sdetail group by sname ");  // Use select query here 

        while($data = mysqli_fetch_array($records))
        {
            echo "<option value='". $data['sname'] ."'>" .$data['sname'] ."</option>";  // displaying data in option menu
        }	
}
    ?>  
</select>
...
<select name="cname" required >
    ...
    <?php
        include "../db.php";  // Using database connection file here
$id=$sessionId;
$query = "SELECT * from cdetail where id='".$id."'"; 
$result = mysqli_query($con, $query) or die ( mysqli_error());
$row = mysqli_fetch_assoc($result);
{
	$apikey = $row['apikey'];

	$records = mysqli_query($con, "SELECT cname From cdetail group by cname ");  // Use select query here 
